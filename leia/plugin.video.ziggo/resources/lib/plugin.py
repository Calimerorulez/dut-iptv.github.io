...
def check_entitlements(**kwargs):
    if plugin.logged_in:
        ...
        if settings.getBool(key='_base_v3') == False:
            ...
            media_groups_url = '{mediagroups_url}/CAT_DATA_VOD__MOVIES_SERIES_XL__Movies__Genres__Actie?byHasCurrentVod=true&range=1-1&sort=playCount7%7Cdesc'.format(mediagroups_url=settings.get('_mediagroupsfeeds_url'))
        ...
        if (not urldata or not check_key(urldata, 'play_url') or not check_key(urldata, 'locator') or urldata['play_url'] == 'http://Playout/using/Session/Service') and settings.getBool(key='_base_v3') == True:
                urldata = {}
