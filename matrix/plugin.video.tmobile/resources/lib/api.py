...
class API(object):
    ...
    def login(self, username, password, channels=False, retry=False):
        ...
        resp = self.download(url=login_url, type="get", code=None, data=None, json_data=False, data_return=True, return_json=False, retry=retry, check_data=False)
        ...
        if self.check_data(resp=resp) == False:
            ...
            resp = self.download(url=login_url, type="post", code=None, data=session_post_data, json_data=False, data_return=True, return_json=False, retry=retry, check_data=False)
        ...
        data = self.download(url='{base_url}/api/info'.format(base_url=CONST_BASE_URL), type="get", code=[200], data=None, json_data=False, data_return=True, return_json=True, retry=retry, check_data=True)
    ...
    def get_channel_data(self, rows, row):
        path = ADDON_PROFILE + "images" + os.sep + str(rows[row]['id']) + ".png"
